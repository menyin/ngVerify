...
(function () {
    ...
    m.service('ngVerify', function () {
        return {
            /***
             * 查找出name=formName并且其_verifyScope存在的form标签
             * @param formName
             * @returns {*}
             */
    ...
    m.directive('verifyScope', function () {
        return {
            ...
            controller: ['$scope', '$element', '$attrs', function ($scope, $element, $attrs) {
                this.getscope = function () {//让子指令能够用require和pCtrl获取此作用域对象
                    return $scope;
                };
            ...
            link: function (scope, iElm, iAttrs, pCtrl) {
                ...
                setTimeout(function () {
                    ...
                    if (pCtrl != undefined) { // 在作用域内
                        ...
                        //获取对应的父指令作用域$scope
                        pScope = ngVerify.scope(OPTS.control)//ngVerify是上面定义的服务，获取的是其指定父作用域form元素的_verifyScope属性,即其$scope
    ...
    function Init(iElm) {
        ...
        // 传入错误参数警告并做容错处理//不允许radio在验证指令里指定least参数
        ...
        // 元素初始化数据//自定义的在form父作用域之外的提交按钮或或之内的提交按钮
    ...
    function bindVaild(iElm) {
        ...
        function blurTrigger() {
            // 失去焦点，校验一次，当前元素不通过//不做提示但输入框高亮
            if (!ISVALID(iElm)) {

                tipMsg(iElm, false);//不做提示

                makeError(iElm, true);//但输入框高亮
        ...
        function changeTrigger() {
            ...
            //iElm.ngVerify.invalid在makeError()里若验证不通过就已经置为true
            //因此当第一次点击输入框时，只显示高亮边框而无提示信息
            ...
            scope.$watch(iAttrs.ngModel, function (newValue, oldValue) {
                if (newValue != null || oldValue != null) {
                    ...
                    if (newValue != null && !isEmptyObject(newValue)) {
                        ...
                        //存储iElm.ngVerify.modelValue，因为在ISVALID(iElm)方法中获取val有可能是modelValue
    ...
    function makeError(iElm, draw) {
        ...
        if (iElm[0].type == 'checkbox' || iElm[0].type == 'radio') {
            if (parent[0].localName == 'label') {//localName是xml节点名称
        ...
        if (OPTS.pattern) {
            ...
            switch (iAttrs.type) {
                ...
                case 'phone':
                    ...
                    break;
                case 'tel':
                    pat = "^(0\\d{2}-\\d{8}(-\\d{1,4})?)|(0\\d{3}-\\d{7,8}(-\\d{1,4})?)$";
                    pat.name = '固定电话';
                    break;
